...
from gc.crypto.QRandomX import QRandomX
...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_default_balance_size, set_hard_fork_block_number, \
    set_gc_dir, replacement_getTime, get_some_address, gen_blocks
...
class TestChainManagerReal(TestCase):
    ...
    def test_load_twice(self):
        ...
        m_fork_state = Mock(autospec=gcstateinfo_pb2.ForkState, initiator_headerhash=self.genesis_block.headerhash)
    ...
    @patch("gc.core.DifficultyTracker.DifficultyTracker.get")
    def test_add_block(self, mock_difficulty_tracker_get):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            with patch('gc.core.misc.ntp.getTime') as time_mock:
                time_mock.return_value = 1615270948  # Very high to get an easy difficulty
...
class TestChainManager(TestCase):
    ...
    @patch('gc.core.Block.Block', autospec=True)
    ...
    @patch('gc.core.config')
    @patch('gc.core.ChainManager.ChainManager.re_org_limit', new_callable=PropertyMock)
