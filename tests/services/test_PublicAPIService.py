...
from pygclib.pygclib import str2bin, hstr2bin, bin2hstr
...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, replacement_getTime, set_gc_dir
...
class TestPublicAPI(TestCase):
    ...
    def test_getNodeState(self):
        ...
        gcnode._p2pfactory = p2p_factory
        gcnode._pow = p2p_factory.pow

        service = PublicAPIService(gcnode)
        node_state = service.GetNodeState(request=gc_pb2.GetNodeStateReq, context=None)

        # self.assertEqual(__version__, node_state.info.version)  # FIXME
        self.assertEqual(gc_pb2.NodeInfo.UNSYNCED, node_state.info.state)
    ...
    def test_getKnownPeers(self):
        ...
        gcnode._p2pfactory = p2p_factory
        gcnode._pow = p2p_factory.pow
        gcnode.peer_manager = Mock()
        gcnode.peer_manager.known_peer_addresses = ['127.0.0.1', '192.168.1.1']

        service = PublicAPIService(gcnode)
        response = service.GetKnownPeers(request=gc_pb2.GetKnownPeersReq, context=None)
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            service = PublicAPIService(mock_blockchain.gcnode)
            request = gc_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getAddressState(self):
        with set_gc_dir('no_data'):
            ...
            gcnode.set_chain_manager(chain_manager)
            gcnode._p2pfactory = p2p_factory
            gcnode._peer_addresses = ['127.0.0.1', '192.168.1.1']
    ...
    def test_getObject(self):
        ...
        with set_gc_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           block.block_number,
                                           gc_pb2.BlockNumberMapping(headerhash=block.headerhash),
                                           None)
            context = Mock(spec=ServicerContext)
            request = gc_pb2.GetObjectReq()
    ...
    def test_getLatestData(self):
        with set_gc_dir('no_data'):
            ...
            gcnode = gcNode(mining_address=b'')
            gcnode.set_chain_manager(chain_manager)
            gcnode.get_block_from_index = MagicMock(return_value=None)
            ...
            request = gc_pb2.GetLatestDataReq(filter=gc_pb2.GetLatestDataReq.ALL,
                                               offset=0,
                                               quantity=3)
    ...
    def test_getAddressFromPK(self):
        ...
        request = gc_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    @patch('gc.core.config.DevConfig', new_callable=PropertyMock)
    def test_getMiniTransactionsByAddress(self, mock_dev_config):
        ...
        with set_gc_dir('no_data'):
            ...
            TransactionMetadata.update_tx_metadata(db_state, block, None)
            request = gc_pb2.GetMiniTransactionsByAddressReq(address=alice_xmss.address,
                                                              item_per_page=10,
                                                              page_number=1)
    ...
    def test_getTransaction(self):
        ...
        context = Mock(spec=ServicerContext)
        request = gc_pb2.GetTransactionReq()
    ...
    def test_getTotalBalance(self):
        with set_gc_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = gc_pb2.GetTotalBalanceReq()
    ...
    def test_getOTS(self):
        with set_gc_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = gc_pb2.GetOTSReq(address=alice_xmss.address)
    ...
    def test_getBlock(self):
        with set_gc_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = gc_pb2.GetBlockReq(header_hash=b.headerhash)
    ...
    def test_getBlockByNumber(self):
        with set_gc_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           gc_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = gc_pb2.GetBlockByNumberReq(block_number=b.block_number)
