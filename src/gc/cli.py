...
from pygclib.pygclib import mnemonic2bin, hstr2bin, bin2hstr

from gc.core import config
from gc.core.Wallet import Wallet, WalletDecryptionError
from gc.core.misc.helper import parse_hexblob, parse_qaddress
...
from gc.crypto.xmss import XMSS, hash_functions
from gc.generated import gc_pb2_grpc, gc_pb2

ENV_gc_WALLET_DIR = 'ENV_gc_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return gc_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = gc_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='gc Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_gc_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def gc(ctx, verbose, host, port_pub, wallet_dir, json):
    """
    gc Command Line Interface
    """
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = gc_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@gc.command()
@click.option('--src', type=str, default='', prompt=True, help='signer gc address')
@click.option('--master', type=str, default='', prompt=True, help='master gc address')
@click.option('--addr_to', type=str, default='', prompt=True, help='gc Address receiving this message (optional)')
...
@gc.command()
@click.option('--src', type=str, default='', prompt=True, help='source gc address')
@click.option('--master', type=str, default='', prompt=True, help='master gc address')
...
@gc.command()
@click.option('--src', type=str, default='', prompt=True, help='signer gc address')
@click.option('--master', type=str, default='', help='master gc address')
...
@click.option('--owner', default='', prompt=True, help='Owner gc address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(gc_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.gc/')
    except Exception as e:
...
@gc.command()
@click.option('--owner', default='', prompt=True, help='source gc address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = gc_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = gc_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(gc_pb2.GetNodeStateReq())
