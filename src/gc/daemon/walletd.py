...
from pygclib.pygclib import hstr2bin, mnemonic2bin, bin2hstr, gcHelper

from gc.core import config
from gc.core.AddressState import AddressState
from gc.daemon.helper import logger
from gc.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from gc.services.WalletAPIService import WalletAPIService
from gc.generated import gc_pb2, gc_pb2_grpc, gcwallet_pb2
from gc.generated.gcwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
pid = os.path.join(config.user.wallet_dir, 'gc_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = gc_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = gcwallet_pb2.PlainBlockHeader()
        ...
        pblock = gcwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = gcwallet_pb2.PlainGenesisBalance()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = gc_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def sign_and_push_transaction(self,
        ...
        push_transaction_req = gc_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
        push_transaction_resp = self._public_stub.PushTransaction(push_transaction_req, timeout=CONNECTION_TIMEOUT)
        if push_transaction_resp.error_code != gc_pb2.PushTransactionResp.SUBMITTED:
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = gc_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = gc_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(gc_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            gc_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(gc_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(gc_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(gc_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(gc_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(gc_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(gc_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(gc_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)

    def get_address_from_pk(self, pk: str) -> str:
        return self.address_to_qaddress(gcHelper.getAddress(bytes(hstr2bin(pk))))

    def get_node_info(self):
        return self._public_stub.GetNodeState(gc_pb2.GetNodeStateReq())
...
def main():
    ...
    daemon = Daemonize(app="gc_walletd", pid=pid, action=run)
    daemon.start()
